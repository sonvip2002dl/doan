...
{      			    
    ...
    {   
		if(size==12)temp=asc2_1206[chr][t]; 	 	//调用1206字体
			...
			else if(size==16)temp=a_1616[chr][t];	//调用1608字体
...
void OLED_Showzi(u8 x,u8 y,u8 chr,u8 size,u8 mode)
{
	u8 temp,t,t1;
	u8 y0=y;
    ...
    for(t=0;t<csize;t++)
    {   
			if(size==16)temp=a_1616[chr][t];	//调用1608字体
			else return;								//没有的字库
        for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)OLED_DrawPoint(x,y,mode);
...
void OLED_Showhanzi(u8 x,u8 y,u8 num,u8 size)
{
	for(;num<11;num++)
	{
		if(x>(128-(size/2))){x=0;y+=size;}
		if(y>(64-size)){y=x=0;OLED_Clear();}
		OLED_Showzi(x,y,num,size,1);	 
		x+=size;
	}
}
//初始化SSD1306					    
