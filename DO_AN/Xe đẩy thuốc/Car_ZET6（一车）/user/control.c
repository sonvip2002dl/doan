		  ...
		  Light_GoStraight_control();    //一直获取灰度传感器状态，但只有在执行car_go的时候，才进行补偿
			...
			{	
				...
				//其实这个要是用好一点的驱动模块的话，短时间内运行也不需要.....不用怕电机驱动过热...一直PID控制也没啥
					...
					if( (Motor_journey_cm <= (g_fTargetJourney + 24)) && (Motor_journey_cm >= (g_fTargetJourney - 24)) )  //这里偏差这么多真的好吗？并不是真的偏差
					{
...
void Car_go(int32_t location_cm)   //直走函数     //连续两次的直行好像会让巡线补偿失效
{
	...
	//如果巡线效果不好就将3.142加大
	Car_location   = ( location_cm / (2 * WheelR * 3.1416) ) *   (REDUCTION_RATIO*ENCODER_TOTAL_RESOLUTION) ;    //Car_location 将  location_cm 转换为对应的脉冲数   g_fTargetJourney = 要转多少圈 * 1圈的脉冲数 
...
void spin_Turn(spin_dir_t zhuanxiang)   //传入小车的轮距(mm) 175mm     //其实转向环并不需要很精准，转弯后直接用直走时的巡线函数回正车身就好
{
    ...
    Spin_start_flag = 1;   
	  ...
	  /**************要是转弯后不能回到线上，可以改这里的转向系数**************/
		...
		{
			Car_Turn_val = ( spin90_val / (2 * WheelR * 3.1416) ) *   (REDUCTION_RATIO*ENCODER_TOTAL_RESOLUTION) ; 
