...
/****************左边电机引脚初始化**************/
/* 设置速度（占空比） */
#define SET_FWD_COMPAER(ChannelPulse)     __HAL_TIM_SET_COMPARE(&htim5,TIM_CHANNEL_3,ChannelPulse)    // 设置比较寄存器的值
#define SET_REV_COMPAER(ChannelPulse)     __HAL_TIM_SET_COMPARE(&htim5,TIM_CHANNEL_4,ChannelPulse)    // 设置比较寄存器的值

/* 使能输出 */
#define MOTOR_FWD_ENABLE()      HAL_TIM_PWM_Start(&htim5,TIM_CHANNEL_3);  //AIN1     PA2
#define MOTOR_REV_ENABLE()      HAL_TIM_PWM_Start(&htim5,TIM_CHANNEL_4);   //AIN2     PA3

/* 禁用输出 */
#define MOTOR_FWD_DISABLE()     HAL_TIM_PWM_Stop(&htim5,TIM_CHANNEL_3);
#define MOTOR_REV_DISABLE()     HAL_TIM_PWM_Stop(&htim5,TIM_CHANNEL_4);



/****************右边电机引脚初始化**************/
/* 设置速度（占空比）2 */
#define SET2_REV_COMPAER(ChannelPulse)       __HAL_TIM_SET_COMPARE(&htim8,TIM_CHANNEL_1,ChannelPulse)    // 设置比较寄存器的值  
#define SET2_FWD_COMPAER(ChannelPulse)       __HAL_TIM_SET_COMPARE(&htim8,TIM_CHANNEL_2,ChannelPulse)    // 设置比较寄存器的值


/* 使能输出2 */
#define MOTOR2_REV_ENABLE()                  HAL_TIM_PWM_Start(&htim8,TIM_CHANNEL_1);    // BIN1    PC6
#define MOTOR2_FWD_ENABLE()                  HAL_TIM_PWM_Start(&htim8,TIM_CHANNEL_2);    // BIN2    PC7

/* 禁用输出2 */
#define MOTOR2_REV_DISABLE()                 HAL_TIM_PWM_Stop(&htim8,TIM_CHANNEL_1);     
#define MOTOR2_FWD_DISABLE()                 HAL_TIM_PWM_Stop(&htim8,TIM_CHANNEL_2);    
